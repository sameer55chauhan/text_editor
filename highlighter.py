...
class Highlighter:
    def __init__(self, text_widget, syntax_file):
        ...
        self.keywords = ['True', 'False', 'def', 'for', 'while', 'import', 'if', 'else', 'elif']
        self.disallowed_previous_chars = ['_', '-', '.']

        self.parse_syntax_file()
        self.text_widget.bind('<KeyRelease>', self.on_key_release)

        # self.tag_configure('keyword', foreground=self.keyword_color)
        # self.tag_configure('number', foregound=self.numbers_color)
        # self.text_widget.bind('<KeyRelease>', self.on_key_release)
