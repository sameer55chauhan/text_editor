...
class ColorChooser(tk.Toplevel):
    def __init__(self, master, **kwargs):
        ...
        self.all_frames = [window_frame, window_background_frame, window_foreground_frame,
                           text_frame, text_background_frame, text_foreground_frame]
        ...
        foreground_color_chooser = ttk.Button(window_foreground_frame, text='Change Foreground Color', width=26, style='editor.TButton', command=lambda sv=self.chosen_fg_color: self.set_color(sv))
        background_color_chooser = ttk.Button(window_background_frame, text='Change Background Color', width=26, style='editor.TButton', command=lambda sv=self.chosen_bg_color: self.set_color(sv))
        text_foreground_color_chooser = ttk.Button(text_foreground_frame, text="Change Text Foreground color", width=26, style="editor.TButton", command=lambda sv=self.chosen_text_fg_color: self.set_color(sv))
        text_background_color_chooser = ttk.Button(text_background_frame, text="Change Text Background color", width=26, style="editor.TButton", command=lambda sv=self.chosen_text_bg_color: self.set_color(sv))

        foreground_color_preview = ttk.Label(window_foreground_frame, textvar=self.chosen_fg_color, style="editor.TLabel")
        background_color_preview = ttk.Label(window_background_frame, textvar=self.chosen_bg_color, style="editor.TLabel")
        text_foreground_color_preview = ttk.Label(text_foreground_frame, textvar=self.chosen_text_fg_color, style="editor.TLabel")
        text_background_color_preview = ttk.Label(text_background_frame, textvar=self.chosen_text_bg_color, style="editor.TLabel")
    ...
    def save(self):
        yaml_file_contents = f"background: '{self.chosen_bg_color.get()}'\n" \
                             + f"foreground: '{self.chosen_fg_color.get()}'\n" \
                             + f"text_background: '{self.chosen_text_bg_color.get()}'\n" \
                             + f"text_foreground: '{self.chosen_text_fg_color.get()}'\n"

        with open(self.master.default_scheme_pathse, "w") as yaml_file:
            yaml_file.write(yaml_file_contents)

        self.master.apply_color_scheme(self.chosen_fg_color.get(), self.chosen_bg_color.get(),
                                       self.chosen_text_fg_color.get(), self.chosen_text_bg_color.get())
